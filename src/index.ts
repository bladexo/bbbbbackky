...
// Identity-to-user mapping for reconnection support
const userIdentities = new Map<string, string>(); // Maps identity to socket.id
...
io.on('connection', async (socket) => {
  ...
  socket.on('check_hack_access', async ({ userId }, callback) => {
    ...
    try {
      const hackAccess = await HackAccess.findOne({ username: user.username, isActive: true });
      ...
      console.error('[HACK] Error checking access:', error);
      callback({ hasAccess: false });
    }
  });
